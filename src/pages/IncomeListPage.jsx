...
export default function IncomeListPage() {
  ...
  const query = useMemo(
    ...
    [filters.page, filters.categoryId, filters.startDate, filters.endDate, filters.minAmount, filters.maxAmount, filters.sortBy, filters.sortOrder, search]
  );

  useEffect(() => {
    categoryApi.list({ type: 'income' }).then(({ data }) => setCategories(data.data.categories || []));
  }, []);
  ...
  const confirmDelete = async () => {
    try {
      await incomeApi.remove(deleteItem._id);
      setRows((items) => items.filter((item) => item._id !== deleteItem._id));
      notify('Income deleted');
    } catch (error) {
      notify(getErrorMessage(error, 'Unable to delete income'), 'error');
    } finally {
      setDeleteItem(null);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">Search, filter, and manage income records.</p>
        </div>
        <Link to="/income/new">
      ...
      <Card>
        ...
        {loading ? (
          ...
          <TransactionTable
            ...
            emptyDescription="Add your first income transaction to start tracking your finances."
            emptyAction={
              <Link to="/income/new">
                <Button>Add income</Button>
              </Link>
      ...
      <Modal open={Boolean(viewItem)} title="Income details" onClose={() => setViewItem(null)}>
        {viewItem ? (
          <dl className="space-y-2 text-sm">
            ...
            <div className="flex justify-between"><dt>Source</dt><dd>{viewItem.source}</dd></div>
      ...
      <Modal open={Boolean(deleteItem)} title="Delete income?" onClose={() => setDeleteItem(null)}>
